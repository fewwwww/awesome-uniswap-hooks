...
- [Uniswap Hook Official Allowlist](https://github.com/Uniswap/routing-api/blob/main/lib/util/hooksAddressesAllowlist.ts): A per-chain allowlist of Uniswap V4 hook contract addresses that Uniswap routing API is permitted to route trades through.
...
- [GlueHook](https://gluehook.trade/): Free general-purpose hook that gives any pool a permissionless buyback pot (buys behind every swap in both directions, sized by the pool's own fee × depth, burns what it buys) and a self-compounding LP with configurable fee splits. Keyless, non-upgradeable, 0% protocol fee, same address on 13 networks.
...
- [Uniswap v4 hooks were a mistake ](https://x.com/0xProject/status/2099578032960995502): Exposes how malicious Uniswap v4 hooks spoof aggregators by quoting one price and settling at another, with analysis of 84,163 hooks finding only 19.4% safe.
...
- [Simple AMM in Optimization Arena](https://www.optimizationarena.com/amm): Dan Robinson & Benedict Brady's AMM Challenge that everyone can submit Uniswap v4-style Solidity hooks for dynamic fees, compete to maximize Edge against a 30bps benchmark in randomized simulations, and top the leaderboard
